...
# Script to run all microservices using venv

echo "Starting Chart AI Backend Services (venv mode)..."
echo "======================================"
...
if [ ! -d "$PROJECT_ROOT/venv" ]; then
    ...
    echo "Please run ./scripts/setup-venv.sh first"
    exit 1
fi
...
python -m uvicorn app.main:app --reload --port 8001 > /tmp/chart-service.log 2>&1 &
CHART_PID=$!
...
python -m uvicorn app.main:app --reload --port 8002 > /tmp/data-service.log 2>&1 &
DATA_PID=$!
